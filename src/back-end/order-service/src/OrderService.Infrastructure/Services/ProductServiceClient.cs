...
public class ProductServiceClient(HttpClient httpClient, ILogger<ProductServiceClient> logger) : IProductServiceClient
{
    public async Task<ProductInfo?> GetProductAsync(Guid productId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                logger.LogInformation("Product info retrieved successfully: {ProductId}", productId);
                return product;
            }
            
            logger.LogWarning("Product not found: {ProductId} - Status: {StatusCode}", productId, response.StatusCode);
            return null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> ValidateProductAsync(Guid productId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            logger.LogInformation("Product validation result: {ProductId} - {IsValid}", productId, isValid);
            return isValid;
        }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<ProductInfo>> GetProductsAsync(IEnumerable<Guid> productIds, CancellationToken cancellationToken = default)
    {
        try
        {
