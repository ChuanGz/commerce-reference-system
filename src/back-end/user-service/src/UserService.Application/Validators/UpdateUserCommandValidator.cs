    ...
    public UpdateUserCommandValidator(IServiceScopeFactory scopeFactory)
    {
        ...
        RuleFor(x => x).CustomAsync(async (command, context, cancellationToken) =>
        {
            ...
            var nameExists = await repo.AnyAsync(
                u => u.Id != command.Id && (u.Name ?? "").Trim().ToUpper() == normalizedName,
                cancellationToken);

            if (nameExists)
                context.AddFailure(nameof(command.Name), $"Name `{command.Name}` already exists.");

            var emailExists = await repo.AnyAsync(
                u => u.Id != command.Id && (u.Email ?? "").Trim().ToUpper() == normalizedEmail,
                cancellationToken);

            if (emailExists)
                context.AddFailure(nameof(command.Email), $"Email `{command.Email}` already exists.");
        });
    }
}
